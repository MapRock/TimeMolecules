...
def get_semantic_web_llm_values_df() -> pd.DataFrame:
    ...
    FROM [TimeSolutionsMetadata]
    WHERE ObjectType IN ('SQL_STORED_PROCEDURE', 'SQL_TABLE_VALUED_FUNCTION')
    """
#   WHERE ObjectType IN ('Column','Table','VIEW','SQL_SCALAR_FUNCTION', 'SQL_STORED_PROCEDURE', 'SQL_TABLE_VALUED_FUNCTION', 'SQL_INLINE_TABLE_VALUED_FUNCTION')
...
if __name__ == "__main__":
    ...
    if llm == "openai":
        ...
        client = QdrantClient(path="c:/MapRock/TimeMolecules/qdrant_data_openai")
        print("✅ Using OpenAI for embeddings.")
    elif llm == "ollama":
        EMBED_MODEL = os.getenv("OLLAMA_EMBED_MODEL", "nomic-embed-text")
        OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", None)
        client = QdrantClient(path="c:/MapRock/TimeMolecules/qdrant_data_ollama")
    ...
    try:
        ...
        if not client.collection_exists(collection_name=collection_name):
            ...
            df = get_semantic_web_llm_values_df()
            print(f"✅ Retrieved {len(df)} rows from Time Molecules database.")
